...
test_misclassification_detection = function(testing_res, pop_model, depth_cutoff, plot_flag = FALSE){
  ...
  if (plot_flag){
    ...
    plot_save(cowplot::plot_grid(p1,p2,p3,ncol = 3), "CSPDS_Test", bh=6.5, asp = 3)
    plot_save(p1, "CSPDS_Test_SB", bh=5, asp = 2)
...
build_pds_model = function(pri_calib_df,sec_calib_df,pds_mean_win,ncomp,lst="class", plot_flag = FALSE){
  ...
  if (plot_flag){
    ...
    plot_save(cowplot::plot_grid(p2,p3,ncol = 2), "PDS_PCA", bh=6.5,asp = 2)
...
test_pds_model = function(test_df, simple_pds_model, plot_flag = FALSE){
  ...
  if (plot_flag){
    ...
    plot_save(cowplot::plot_grid(p1,p2,ncol = 2), "PDS_Test", bh = 6.5,asp = 2)
  }
  return(adj_test_df)
}
...
build_cca_model = function(pri_calib_df,sec_calib_df,k,lst="class", plot_flag=FALSE){
  ...
  if (plot_flag){
    p1 = plot_line(data.frame(as.numeric(colnames(pri_calib_df$NIR)),
                                      ...
                                      secondary_spec = sec_calib_df$NIR[1,],
                                      adj_secondary_spec = adj_sec_calib_df$NIR[1,]))
    plot_save(p1, "Simple_Model_CCA_spec", asp = 2)
    type = append(rep("Primary Calib", nrow(pri_calib_df)), 
                  rep("Secondary Calib", nrow(sec_calib_df)))
    original = rbind(pri_calib_df,sec_calib_df)
    p2 = plot_pca(prcomp(original$NIR, TRUE), 
    ...
    plot_save(cowplot::plot_grid(p2,p3,ncol = 2), "Simple_Model_CCA_PCA", bh = 6.5,asp = 3)
  }
...
test_cca_model = function(test_df, simple_ctcca_model, plot_flag=FALSE){
  ...
  if (plot_flag){
    ...
    #plot_save(cowplot::plot_grid(p1,p2,ncol = 2), "Simple_Transfer_CCA_Test", bh=6.5,asp = 2)
  }
  return(adj_test_df)
}
...
build_cspds_model = function(pri_calib_df,sec_calib_df,pds_mean_win,ncomp,lst="class", plot_flag = TRUE){
  ...
  if (plot_flag){
    ...
    p3 = plot_pca(prcomp(pri_adj_sec_calib_df$NIR, TRUE), 
                  color_group = type, shape_group = pri_adj_sec_calib_df[[lst]])
    plot_save(cowplot::plot_grid(p2,p3,ncol = 2), "CSPDS_PCA", bh=6.5, asp = 2)
...
build_cscca_model = function(pri_calib_df,sec_calib_df,k,lst="class", plot_flag){
  ...
  if (plot_flag){
    p1 = plot_line(data.frame(as.numeric(colnames(pri_calib_df$NIR)),
                                      ...
                                      secondary_spec = sec_calib_df$NIR[1,],
                                      adj_secondary_spec = adj_sec_calib_df$NIR[1,]))
    plot_save(p1, "CSCCA_Spec_1",asp=2)
    ...
    p3 = plot_pca(prcomp(pri_adj_sec_calib_df$NIR, TRUE), 
                  color_group = type, shape_group = pri_adj_sec_calib_df[[lst]])
    plot_save(cowplot::plot_grid(p2,p3,ncol = 2), "CSCCA_1_PCA", bh=6.5, asp = 2)
...
build_cspcpds_model = function(pri_calib_df,sec_calib_df,pds_mean_win,ncomp,npc,lst="class", plot_flag = TRUE){
  ...
  if (plot_flag){
    p1 = plot_line(data.frame(as.numeric(colnames(pri_calib_df$NIR)),
                                      ...
                                      secondary_spec = sec_calib_df$NIR[1,],
                                      adj_secondary_spec = adj_sec_calib_df$NIR[1,]))
    plot_save(p1, "cspcpds_Spec",asp=2)
    ...
    p3 = plot_pca(prcomp(pri_adj_sec_calib_df$NIR, TRUE), 
                  color_group = type, shape_group = pri_adj_sec_calib_df[[lst]])
    plot_save(cowplot::plot_grid(p2,p3,ncol = 2), "cspcpds_PCA", bh=6.5, asp = 2)
...
plot_save = function(p, name, bh = 5, asp = 1.5){
  fname = paste0(plot_results_dir, "/", plot_counter,"_",name, ".png")
  cowplot::save_plot(fname, p,  base_height = bh, base_asp = asp)
  assign("plot_counter", (plot_counter+1), envir = .GlobalEnv)
}

plot_line = function(df, ll=c("Simple Plot", expression(paste("Wavenumber (", cm^{-1},")")),
                                      "Absorbance", "Specs")) {
...
plot_dot = function(df, ll=c("Simple Plot", expression(paste("Wavenumber (", cm^{-1},")")),
                              "Absorbance", "Specs")) {
  #using package reshape2, ggplot2
  col_df = colnames(df)
  col_df[1] = "x"
...
plot_bar = function(df, ll=c("Simple Plot", "Wavenumber", "Absorbance", "Specs")) {
  col_df = colnames(df)
  col_df[1] = "x"
  colnames(df) = col_df
  df_m = reshape2::melt(df, id.vars = "x")
...
plot_scatter = function(df, quant=FALSE, ll = c("Simple Scatter Plot","Comp1","Comp2",
...
plot_pca = function(pca, c1=1, c2=2, quant=FALSE, color_group, shape_group,
...
plot_heatmap = function(df, ll =  c("Heatmap", "Samples", "Population")){
  df$row = rownames(df)
  ...
  p = ggplot2::ggplot(data = melted_df, ggplot2::aes(x = row, y = variable, fill = value)) +
    ...
    ggplot2::scale_fill_gradient(low = "#34A853", high = "#EA4335")+
    #ggplot2::scale_fill_gradient(high = "#34A853", low = "#EA4335")+
    #ggplot2::cood_fixed()+
